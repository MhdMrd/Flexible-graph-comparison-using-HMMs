...
#include <dirent.h>
#include <time.h>
#include "arff_binary_vL.h"

/*
	arffGraphs_binary_vL.exe   DirectoryName(IMDB-BINARY-vL) NbProperties(1) epsilon(0.0) 
	              MaxIterations(100) Threshold(100.0) OutputFile(Output.txt)
				  SaveInitHMMs(0) SaveFinalHMMs(0) DepthMaxDFS (1)
*/
int main(int NbArg, char ** MesArg) {
  int Class,j,vertex,idGraph,NbGraphs[MaxClasses],IndexProp,T[MaxVertices];
  ...
  while(NbVertices != 0){
	...
	BuildMCs(graph,MCgraph,Candidates,NbProperties,DepthMaxDFS,T,&f,NbMCs,LabelList);
	
	InitialHMMs(MCgraph,LambdaInit,NbMCs,DepthMaxDFS,NbProperties,NbLabels);
