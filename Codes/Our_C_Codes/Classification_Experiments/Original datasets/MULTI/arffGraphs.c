...
/*
	arffGraphs.exe    DirectoryName(graphs) NbProperties(2) epsilon(0.0) 
	              MaxIterations(100) Threshold(100.0) OutputFile(Output.txt)
				  SaveInitHMMs(0) SaveFinalHMMs(0)  NbSlicesNeighborhood(10) DepthMaxDFS (2)
*/
int main(int NbArg, char ** MesArg) {
  int Class,j,vertex,idGraph,NbGraphs[MaxClasses],IndexProp,T[MaxVertices],NbSlicesNeighborhood;
  ...
  fprintf(f,"\n**************************** class c%i *****************************\n",Class + 1); 	
  printf("\n\n__________ class %i ____________\n",Class+1);
  ...
  idGraph = 0;
  GetNextGraph(idGraph,&g,&NbVertices,Edges,&NbEdges,&f);

  while(NbVertices != 0){
	graph = createGraph(NbVertices);
	...
	BuildMCs(graph,MCgraph,Candidates,NbProperties,DepthMaxDFS,T,&f,NbSlicesNeighborhood,NbMCs);
	
	InitialHMMs(MCgraph,LambdaInit,NbMCs,DepthMaxDFS,NbProperties,NbSlicesNeighborhood);
